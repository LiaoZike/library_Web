...
<div class="ifcontent container-fluid">
    ...
    <div class="row">
        ...
        <div class="floorName">樓層：{{$floorname}}</div>
    ...
    <div class="row mt-2" style="align-items: center;">
        <div class="col-12 col-md-5">
            <div class="col-12">
                層數：<input disabled name="severalrows" id="severalrows" style="width:calc(100% - 70px)" type="number" min="0" value="{{$severalrows}}"/>
            </div>
        </div>
        <div class="col-12 col-md-5">
            <div class="col-12">
                行數：<input disabled name="severalcols" id="severalcols"  style="width:calc(100% - 70px)"  type="number" min="0" value="{{$severalcols}}"/>
            </div>
        </div>
    ...
    @if(isset($BookCaseNos[$j * $severalrows + $i + 1]))
@endif
    <a href="{{route('admin.inventory.end',['gotopid'=>$floormapid,'timesname'=>$timesname,'floor'=>$floorname,'floormap'=>$floormapname,'caseno'=>$j*$severalrows+$i+1])}}" class="BlockCol">
        <div class="NumberBlock @if(isset($BookCaseNos[$j * $severalrows + $i + 1]) && $BookCaseNos[$j * $severalrows + $i + 1]['errorcount']['errorct']!=0) bck_red
        @elseif(isset($BookCaseNos[$j * $severalrows + $i + 1]) && $BookCaseNos[$j * $severalrows + $i + 1]['errorcount']['correct']!=0) bck_green @else bck_gray @endif">
...
{{--                            <label>起始編碼</label>--}}
                <input readonly class="disabled_input" style="width: 95%;" value="{{isset($BookCaseNos[$j * $severalrows + $i + 1]) ? $BookCaseNos[$j * $severalrows + $i + 1]->startnum : ""}}" type="text" name="startnum[]" placeholder="xxx xxxx xxxx"/>
            </div>
            <div style="text-align: center;">
                <label style="rotate:90deg">~</label>
            </div>
            <div class="StartNumber">
{{--                           <label>結束編碼</label>--}}
                <input readonly class="disabled_input"  style="width: 95%;" value="{{isset($BookCaseNos[$j * $severalrows + $i + 1]) ? $BookCaseNos[$j * $severalrows + $i + 1]->endnum : ""}}" type="text" name="endnum[]" placeholder="xxx xxxx xxxx"/>
            </div>
            <h6 style="text-align: right">
            @if(isset($BookCaseNos[$j * $severalrows + $i + 1])  && $BookCaseNos[$j * $severalrows + $i + 1]['errorcount']['errorct']!=0) {{$BookCaseNos[$j * $severalrows + $i + 1]['errorcount']['errorct']}} @else .@endif
            </h6>
        </div>
    </a>
@endfor
...
.times,.floorName,.bookcaseName,.bookcaseNote{
